...
Dim Read_Character_Buffer As String * READ_BUFFER_LENGTH  ' Important - read character buffer must be fixed length.

With COM_PORT
  
 If .Started Then
               
    Read_Result = Synchronous_Read(.Handle, Read_Character_Buffer, READ_BUFFER_LENGTH, Characters_Processed)
            
    If Read_Result And Characters_Processed > LONG_0 Then
    
        Read_Character_String = Left$(Read_Character_Buffer, Characters_Processed)
